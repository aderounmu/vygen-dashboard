...
import { Plus, Trash2, ToggleLeft, ToggleRight, Save, Globe, Shield, ShieldAlert, ShieldCheck, X, ListFilter, AlertTriangle, CheckCircle, Ban } from 'lucide-react';
...
export const Policies: React.FC = () => {
  ...
  const handleAddPolicy = (e: React.FormEvent) => {
    ...
    setNewPolicy({ name: '', description: '', data_type: '', action: ActionType.BLOCK, priority: 1, is_enabled: true });
  };
  ...
  const getActionIcon = (action: ActionType) => {
    switch (action) {
        case ActionType.BLOCK: return <Ban className="w-4 h-4 text-red-600" />;
        case ActionType.WARN: return <AlertTriangle className="w-4 h-4 text-yellow-600" />;
        case ActionType.PASS: return <CheckCircle className="w-4 h-4 text-emerald-600" />;
        default: return null;
    }
  };
  ...
  const getActionStyles = (action: ActionType) => {
    switch (action) {
        case ActionType.BLOCK: return 'bg-red-50 border-red-200 text-red-700 dark:bg-red-900/20 dark:border-red-800 dark:text-red-400';
        case ActionType.WARN: return 'bg-yellow-50 border-yellow-200 text-yellow-700 dark:bg-yellow-900/20 dark:border-yellow-800 dark:text-yellow-400';
        case ActionType.PASS: return 'bg-emerald-50 border-emerald-200 text-emerald-700 dark:bg-emerald-900/20 dark:border-emerald-800 dark:text-emerald-400';
  ...
  return (
    <div className="space-y-10 max-w-5xl mx-auto pb-20">
      ...
      <div className="flex justify-between items-center">
        <div>
            <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Governance & Policies</h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1">Manage AI platform access and data protection rules.</p>
        </div>
      </div>
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
                ...
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">AI Platforms</h2>
            </div>
            <button 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {state.platforms.map((platform) => (
                <div key={platform.id} className="bg-white dark:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start">
                        <div className="flex items-center space-x-3">
                            <div className={`p-2 rounded-lg ${platform.is_allowed ? 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20' : 'bg-red-50 text-red-600 dark:bg-red-900/20'}`}>
                                {platform.is_allowed ? <ShieldCheck className="w-5 h-5" /> : <ShieldAlert className="w-5 h-5" />}
                            </div>
                            <div>
                                <h3 className="font-semibold text-slate-900 dark:text-white">{platform.tool_name}</h3>
                                <p className="text-xs text-slate-500 dark:text-slate-400 font-mono">{platform.domain}</p>
                            </div>
                        </div>
                        <div className="flex items-center space-x-1">
                            ...
                            >
                                {platform.is_allowed ? <ToggleRight className="w-6 h-6" /> : <ToggleLeft className="w-6 h-6" />}
                            </button>
                            <button 
      ...
      <section className="space-y-4">
        <div className="flex justify-between items-center">
            <div className="flex items-center space-x-2">
                ...
                <h2 className="text-lg font-semibold text-slate-900 dark:text-white">DLP Policies</h2>
            </div>
            <button 
        ...
        <div className="grid gap-4">
            {state.policies.sort((a, b) => a.priority - b.priority).map((policy) => (
                <div key={policy.id} className={`bg-white dark:bg-slate-800 rounded-xl border transition-all ${policy.is_enabled ? 'border-slate-200 dark:border-slate-700 shadow-sm' : 'border-slate-100 dark:border-slate-800 opacity-60'}`}>
                    <div className="p-5">
                        <div className="flex items-start justify-between">
                            <div className="flex items-center space-x-3">
                                <div className={`p-2 rounded-lg ${policy.is_enabled ? 'bg-brand-50 text-brand-600 dark:bg-slate-700 dark:text-brand-400' : 'bg-slate-100 text-slate-400 dark:bg-slate-800'}`}>
                                    <ListFilter className="w-5 h-5" />
                                </div>
                                <div>
                                    <div className="flex items-center space-x-2">
                                        <h3 className="text-base font-semibold text-slate-900 dark:text-white">{policy.name}</h3>
                                        <span className="px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 text-[10px] font-bold rounded uppercase tracking-wider">Priority {policy.priority}</span>
                                    </div>
                        ...
                        <div className="mt-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg p-3 border border-slate-200 dark:border-slate-700/50">
                            <div className="flex flex-wrap items-center gap-2 text-sm">
                                <span className="font-mono text-[10px] text-slate-500 uppercase tracking-widest">IF</span>
                                <div className="px-2 py-1 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded text-slate-700 dark:text-slate-200 font-medium text-xs">
                                    Data contains <span className="text-brand-600 font-bold">{policy.data_type}</span>
                                </div>
                                <span className="font-mono text-[10px] text-slate-500 uppercase tracking-widest">THEN</span>
                                <div className={`px-2 py-1 border rounded font-bold flex items-center gap-1.5 text-xs uppercase tracking-tight ${getActionStyles(policy.action)}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">Add AI Platform</h2>
                    <button onClick={() => setShowPlatformModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                <form onSubmit={handleAddPlatform} className="p-6 space-y-4">
                    <div>
                        ...
                        <input 
                            ...
                            onChange={(e) => setNewPlatform({ ...newPlatform, tool_name: e.target.value })}
                        />
                    </div>
                    <div>
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                        <div>
                            <p className="text-sm font-medium text-slate-900 dark:text-white">Allow Access</p>
                            <p className="text-xs text-slate-500 dark:text-slate-400">Enable users to access this platform</p>
                        </div>
                        <button 
                            type="button"
                            onClick={() => setNewPlatform({ ...newPlatform, is_allowed: !newPlatform.is_allowed })}
                            className="text-brand-600"
                        >
                            {newPlatform.is_allowed ? <ToggleRight className="w-8 h-8" /> : <ToggleLeft className="w-8 h-8 text-slate-400" />}
                        </button>
                    </div>
      ...
      {showPolicyModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-xl w-full max-w-md border border-slate-200 dark:border-slate-700 overflow-hidden">
                <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center">
                    <h2 className="text-xl font-bold text-slate-900 dark:text-white">New DLP Policy</h2>
                    <button onClick={() => setShowPolicyModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200">
                        <X className="w-6 h-6" />
                    </button>
                </div>
                <form onSubmit={handleAddPolicy} className="p-6 space-y-4">
                    <div>
                        ...
                        <input 
                            type="text"
                            required
                            placeholder="e.g. NIN Protection"
                            className="w-full px-4 py-2 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 outline-none"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input 
                                ...
                                onChange={(e) => setNewPolicy({ ...newPolicy, data_type: e.target.value })}
                            />
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <select 
                            ...
                            onChange={(e) => setNewPolicy({ ...newPolicy, action: e.target.value as ActionType })}
                        >
                    ...
                    <div className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-900 rounded-lg">
                        <div>
                            ...
                            <p className="text-xs text-slate-500 dark:text-slate-400">Enable or disable this policy</p>
                        </div>
                        <button 
                            type="button"
                            onClick={() => setNewPolicy({ ...newPolicy, is_enabled: !newPolicy.is_enabled })}
                            className="text-brand-600"
                        >
                            {newPolicy.is_enabled ? <ToggleRight className="w-8 h-8" /> : <ToggleLeft className="w-8 h-8 text-slate-400" />}
                        </button>
                    </div>
                    <div className="pt-4 flex space-x-3">
                        <button 
