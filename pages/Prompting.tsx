      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Simulate prompts across different AI platforms with real-time DLP.
          </p>
        </div>
          ...
          <div className="relative">
            ...
            >
              {(plaforms?.data?.data ?? []).map((p) => (
                <option key={p.id} value={p.tool_name}>
                  {p.tool_name}
                </option>
              ))}
            </select>
        ...
        <div className="space-y-4">
          {!result && !error && (
            <div className="h-full flex flex-col items-center justify-center p-12 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 text-center min-h-[400px]">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-sm max-w-[240px]">
                Results will appear here after validation. We'll check for PII,
                NIN, and other sensitive data.
              </p>
            </div>
          )}
          ...
          {result && (
            <div className="bg-white dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden animate-in slide-in-from-right duration-300">
              {/* Header Status */}
              <div
                className={`p-8 text-center border-b border-slate-100 dark:border-slate-700 ${
                  result.action === "block"
              ...
              <div className="p-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-[10px] font-medium text-slate-400">
                    JSON:{" "}
                    {`{"risk_score": ${result.risk_score}, "action": "${result.action}", "reasons": "${result.reasons.join(", ")}"}`}
                  </span>
                </div>
                  ...
                  <div className="space-y-3">
                    {result.violations.map((v, i) => (
                      ...
                      >
                        ...
                        <div>
                          <p
                            className={`text-sm font-bold ${v.action === "block" ? "text-red-700 dark:text-red-400" : "text-yellow-700 dark:text-yellow-400"}`}
                          >
                            {v.type} Detected
                          </p>
                ...
                <div className="mt-6 pt-6 border-t border-slate-100 dark:border-slate-700">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                    The prompt was analyzed using the{" "}
                    <strong>Vyken DLP Engine</strong>. Detected patterns for{" "}
                    <strong>{result.reasons.join(", ")}</strong> triggered a{" "}
                    <strong>{result.action}</strong> action. Risk score was
                    calculated based on data sensitivity and platform
                    reputation.
                  </div>
                </div>
