...
export const Departments: React.FC = () => {
  ...
  const canUpdateRoles = useHasPermission(
    ["can-unassign-permission-from-role", "can-assign-permission-to-role"],
    state?.organization?.id ?? "",
    true,
  );

  const canCreateRoles = useHasPermission(
  ...
  const { data, isLoading } = useGetBusinessRoles(
    state?.organization?.id ?? "",
    itemsPerPage,
    page,
  );
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 dark:text-slate-400 mt-1">
            Manage organizational units and their access levels.
          </p>
        </div>
        {canCreateRoles && (
          <button
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
              {canSeeRoles ? (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-wrap gap-1.5">
                        {dept.permissions.slice(0, 3).map((p, index) => (
                          <span
                            key={`${index}__permission_${p.name}`}
                ...
                <div className="p-8 text-center text-slate-500">
                  ...
                  <p>Sorry you dont have access to view departments</p>
                </div>
              )}
            </tbody>
          </table>
