...
export const Dashboard: React.FC = () => {
  ...
  const toolData = useMemo(() => {
    ...
    if (!topTools?.data || !topTools?.data?.data) return { value: [], total };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-700/50">
          ...
          <div className="h-[200px] relative">
            ...
            <div className="absolute bottom-0 left-0 right-0 flex justify-center items-end pb-4"></div>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-slate-50 dark:divide-slate-700">
                        ...
                        ].map((row, i) => ( */}
                {
                  ...
                  (currentData?.data?.data ?? []).splice(0, 3).map((row, i) => {
                    ...
                    const riskRate =
                      row.risk_score > 0 && row.risk_score
                        ? (row.risk_score / 10) * 100
                        : 0;
                    return (
                      <tr
                        key={i}
